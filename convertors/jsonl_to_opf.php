<?php
	...
	while(true) {
		...
		if (!$singleWordJson || $wordsCounter === $wordsPerHtml) {
			...
			file_put_contents(
				dirname(__DIR__) . "/opf/bulgarian_dictionary{$htmlsCounter}.html", 
	...
	// Create the OPF file
	file_put_contents(
		dirname(__DIR__) . "/opf/bulgarian_dictionary.opf", 
		renderPhpToString(__DIR__ . '/templates/opf.php', ['manifestItemsCount' => $htmlsCounter])
	);

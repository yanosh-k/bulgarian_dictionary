<?php
	...
	FROM
		...
		LEFT JOIN
		(
			SELECT base_word_id, GROUP_CONCAT(DISTINCT name ORDER BY name ASC SEPARATOR ' @AND@ ') AS derivative_forms
			FROM derivative_form
			WHERE INSTR(name,' ') = 0
			GROUP BY base_word_id
		) derivatives
		ON word_translation.word_id = derivatives.base_word_id
...
";
	...
	$outputDataFile = fopen('bg_en_dictionary.jsonl', 'w');
	
	
	// Go through all words and save them to a jsonl text file

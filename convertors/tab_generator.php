 <?php
	...
	`word`.`name`,
	`word`.`meaning`,
	GROUP_CONCAT(DISTINCT `derivative_form`.`name` ORDER BY derivative_form.`name` ASC SEPARATOR ', ') AS `derivative_forms`
FROM
	`word`
...
";
	...
	$outputDataFile = fopen('bulgarian_dictionary.txt', 'w');
	
	
	// Go trough all words and save the to a .txt file
	foreach ($keyedWords as $singleKeyedWord) {
		fwrite($outputDataFile, 
			$singleKeyedWord['name']
			. "\t" . $singleKeyedWord['id']
			. "\t" . format_meaning($singleKeyedWord['meaning'], $keyedWords) 
			. "\t" . $singleKeyedWord['derivative_forms']
			. "\n"
		);
	}
	...
	function format_meaning($meaning, &$wordsMap)
	{
		...
		$meaning = preg_replace_callback('/\[\[([^]]+?)\]\]/', function($matches) use (&$wordsMap) {
			...
			if (isset($wordsMap[$wordText])) {
				$out = '<a id="' . $wordsMap[$wordText]['id'] . '">' . $wordText . '</a>';
			} else {
				$out = $matches[1];
			}
